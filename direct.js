if (!!!document.getElementById("uploadToast")) {
	...
	if (window.location.hostname === "scratch.mit.edu") {
		if (document.getElementById('report-this') === null) {
			$('<input type="file" id="autothumb" style="display: none;" accept="image/*">').appendTo(document.body);
			document.getElementById("autothumb").addEventListener("change", function () {
				var reader1 = new FileReader();
				reader1.readAsDataURL(document.getElementById('autothumb').files[0]);
				reader1.onload = function () {
					...
					reader.onload = function (e) {
						...
						$.ajax({
							...
							success: function () {
								document.getElementById("uploadToast").innerHTML = 'The thumbnail was successfully changed.<br><img onclick="location.href=\'mystuff\'" title="Click me to show this project in My Stuff" src="' + reader1.result + '" style="background-color:white;height:120px;width:160px;"><br><a onclick="document.getElementById(\'autothumb\').click();">Select another image</a><br><a onclick="document.getElementById(\'uploadToast\').style.display=\'none\';">Close</a>';
							},
							error: function () {
								document.getElementById("uploadToast").innerHTML = 'Error: try again or try with a smaller image.<br><a onclick="document.getElementById(\'autothumb\').click();">Select another image</a><br><a onclick="document.getElementById(\'uploadToast\').style.display=\'none\';">Close</a>';
							}
						});
					};
